...
def snp_extractor():
    ...
    search_string = r"\((SNP\d+)\)"  # no dots or dashes
    results = (
    ...
    for i in results:
        ...
        if sutta_code not in snp_dict:
            snp_dict[sutta_code] = [i.lemma_1]
        else:
            snp_dict[sutta_code] += [i.lemma_1]

    list_of_keys = [k for k in snp_dict]
    file_order = natural_sort(list_of_keys)

    file_path = Path("scripts/suttas/dpd/kn5-snp.tsv")
    file_path.open("w")

    with open(file_path, "a") as f:
        for i in file_order:
            f.write(f"{i}\t{', '.join(snp_dict[i])}\n")
            print(f"{i}\t{snp_dict[i]}")
