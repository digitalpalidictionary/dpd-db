import re

from db.db_helpers import get_db_session
from db.models import DpdHeadword
from tools.paths import ProjectPaths
from tools.sort_naturally import natural_sort

pth: ProjectPaths = ProjectPaths()
db_session = get_db_session(pth.dpd_db_path)


def iti_extractor():
    # search_string = r"\((ITI\d+\.\d+-*\d*)\)" # with dots n dashes
    search_string = r"\((ITI\d+)\)"  # no dots or dashes
    results = (
        db_session.query(DpdHeadword)
        .filter(DpdHeadword.meaning_2.regexp_match(search_string))
        .all()
    )
    iti_dict = {}
    for i in results:
        sutta_code = re.findall(search_string, i.meaning_2)[0]

        if sutta_code not in iti_dict:
            iti_dict[sutta_code] = [i.lemma_1]
        else:
            iti_dict[sutta_code] += [i.lemma_1]

    list_of_keys = [k for k in iti_dict]
    file_order = natural_sort(list_of_keys)

    file_path = pth.suttas_dpd_dir / "kn4-iti.tsv"
    file_path.open("w")

    with open(file_path, "a", encoding="utf-8") as f:
        for i in file_order:
            f.write(f"{i}\t{', '.join(iti_dict[i])}\n")
            print(f"{i}\t{iti_dict[i]}")


if __name__ == "__main__":
    iti_extractor()
