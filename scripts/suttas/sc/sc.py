...
from scripts.suttas.sc.modules import open_sc_json, update_data_dict, write_sc_data_to_tsv
...
def extract_pali(file_path, data):
    ...
    for line_code, pali_text in data.items():
        if folder_name_clean in ["dn", "mn", "kp"]:
            ...
            update_data_dict(code, book, vagga, sutta, data_dict)
        ...
        if folder_name_clean in ["an"] and folder_name in ["an2"]:
            ...
            if ":1.0" in line_code:
                ...
                if not bool(re.match("[0-9-]", sutta)):
                    update_data_dict(
                        code, book, vagga, sutta, data_dict
                    )
