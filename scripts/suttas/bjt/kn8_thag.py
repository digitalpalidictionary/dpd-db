...
def extract_gāthā_data(json_file: Path) -> List[Dict[str, Any]]:
    """Extract gāthā data from kn-thag.json file."""
    results = []
        ...
        for page in pages:
            ...
            for entry in pali_entries:
                ...
                if entry_type == "centered":
                    ...
                    if "theragāthāpāḷi" in text.lower():
                        current_book = text
                    
                    # Check if this is a sutta name heading
                    elif level == 1 and "theragāthā" in text and text != "theragāthāpāḷi":
                        ...
                        if pending_suttas:
                            ...
                            last_sutta_code = max(pending_suttas.keys(), key=lambda x: (
                                pending_suttas[x]["parsed_nipāta"],
                                pending_suttas[x]["parsed_vagga"] if "parsed_vagga" in pending_suttas[x] else 0, 
                            ...
                            # Determine sequential nipata number using global sequence
                            actual_nipata = sutta_info["parsed_nipāta"]
                            if actual_nipata not in global_encountered_nipata_sequence:
                                global_encountered_nipata_sequence[actual_nipata] = global_next_nipata_seq_num
                                global_next_nipata_seq_num += 1
                            
                            sequential_nipata_num = global_encountered_nipata_sequence[actual_nipata]
                            
                            # Use the actual sutta number from the sutta code
                            actual_sutta_number = sutta_info["parsed_sutta"]
                            
                            # Generate web code as kn-thag-{sequential_nipata}-{actual_sutta}
                            web_code = f"kn-thag-{sequential_nipata_num}-{actual_sutta_number}"
                            ...
                            if sutta_info["parsed_nipāta"] <= 2:
                                ...
                                vagga_sutta_counter[vagga_key] = vagga_sutta_counter.get(vagga_key, 0) + 1
                                sutta_num = vagga_sutta_counter[vagga_key]
                                vagga_field = current_vagga
                            else:
                                # Nipātas 3+: increment counter for this nipāta
                                nipāta_sutta_counter[sutta_info["parsed_nipāta"]] = nipāta_sutta_counter.get(sutta_info["parsed_nipāta"], 0) + 1
                                sutta_num = nipāta_sutta_counter[sutta_info["parsed_nipāta"]]
                    ...
                    elif level == 2 and nipāta_num <= 2 and re.match(r"^\d+\.\s*\w+.*vaggo", text):
                        current_vagga = text
...
def main():
    ...
    output_file = Path(
        "/home/bodhirasa/MyFiles/3_Active/dpd-db/scripts/suttas/bjt/kn8_thag.tsv"
    )

    # Find all thag JSON files and sort them properly
    all_files = list(json_dir.glob("*thag*.json"))
    
    # Put kn-thag.json first, then sort others numerically
    main_file = None
    other_files = []
    
    for file in all_files:
        if file.name == "kn-thag.json":
            main_file = file
        else:
            other_files.append(file)
    
    # Sort other files by numeric part (kn-thag-11.json before kn-thag-15.json, etc.)
    def sort_key(file):
        name = file.name
        if "kn-thag-" in name:
            num = name.replace("kn-thag-", "").replace(".json", "")
            try:
                return int(num)
            except:
                return 999
    ...
    thag_files = [main_file] + other_files if main_file else other_files
    ...
    if not thag_files:
        print("No thag JSON files found")
        return

    all_results = []
    
    for json_file in thag_files:
