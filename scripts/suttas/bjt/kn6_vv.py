...
def clean_vimāna_name(text: str) -> str:
    """Clean vimāna name by removing brackets and curly braces only."""
    cleaned = text.replace("[", "").replace("]", "")  # Remove brackets
    cleaned = re.sub(r"\{[^}]*\}", "", cleaned)  # Remove {.*} patterns
    return cleaned.strip()


def extract_vimāna_data(json_file: Path) -> List[Dict[str, Any]]:
    """Extract vimāna data from kn-vv.json file."""
    results = []
        ...
        for page in pages:
            ...
            for entry in pali_entries:
                ...
                if entry_type == "centered":
                    ...
                    if "vimānavatthupāḷi" in text.lower():
                        current_book = text
                    ...
                    # Look for vimāna names in heading entries with level 1
                        ...
                        vagga_match = re.match(r"^(\d+)\.", current_vagga)
                        if vagga_match:
                            vagga_num = int(vagga_match.group(1))
                            # Extract vimāna number by counting existing vimānas in this vagga
                            existing_vimānas = [r for r in results if r["bjt_vagga"] == current_vagga]
                            vimāna_num = len(existing_vimānas) + 1
                            
                            # Generate sutta_code as "{vagga_num}. {vimāna_num}."
                            sutta_code = f"{vagga_num}. {vimāna_num}."

                            # Generate web_code as "kn-vv-{vagga_num}-{vimāna_num}"
                            web_code = f"kn-vv-{vagga_num}-{vimāna_num}"

                            # Clean up vimāna name
                            vimāna_name = clean_vimāna_name(text)
                            ...
                            record = {
                                ...
                                "bjt_minor_section": "",  # Empty for VV (doesn't have nipāta structure)
                                "bjt_vagga": current_vagga,
                                "bjt_sutta": vimāna_name,
                            }
...
def main():
    ...
    json_file = json_dir / "kn-vv.json"
    output_file = Path(
        "/home/bodhirasa/MyFiles/3_Active/dpd-db/scripts/suttas/bjt/kn6_vv.tsv"
    )
    ...
    # Extract vimāna data
    data = extract_vimāna_data(json_file)

    print(f"Extracted {len(data)} vimānas")
