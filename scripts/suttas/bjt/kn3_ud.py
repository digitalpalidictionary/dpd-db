...
def extract_sutta_data(json_file: Path) -> List[Dict[str, Any]]:
    """Extract sutta data from kn-ud.json file."""
    results = []
    ...
    try:
        ...
        for page in pages:
            ...
            # First pass: find vagga headings
            ...
            # Second pass: find sutta numbers
            for entry in pali_entries:
                entry_type = entry.get("type", "")
                text = entry.get("text", "").strip()
                level = entry.get("level", 0)
                ...
                if entry_type == "centered":
                    ...
                    if re.match(r"^\d+\.\s*\d+\.$", text):
                        ...
                        if sutta_match:
                            ...
                            # Generate web_code as "kn-ud-{vagga_num}-{web_sutta_num}"
                            web_code = f"kn-ud-{vagga_num}-{web_sutta_num}"
                            ...
                            record = {
                                ...
                                "bjt_minor_section": "",  # Empty for UD
...
def main():
    ...
    json_file = json_dir / "kn-ud.json"
    output_file = Path(
        "/home/bodhirasa/MyFiles/3_Active/dpd-db/scripts/suttas/bjt/kn3_ud.tsv"
    )
