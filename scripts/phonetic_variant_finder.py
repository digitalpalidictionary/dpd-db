...
"""Find all words with same construction but different pali_1."""
...
class PhoneticVariant():
    def __init__(self, i: PaliWord, dict):
        self.pali_1 = i.pali_1
        self.pali_clean = i.pali_clean
...
class DictEntry():
    def __init__(self, var: PhoneticVariant) -> None:
        ...
        self.headwords: list[str] = [var.pali_1]
        self.headwords_count = 1
        self.headwords_clean: set[str] = set([var.pali_clean])
...
def dict_entry_update(dict, var: PhoneticVariant) -> dict:
    ...
    if not dict.get(var.key):
        ...
        dict_entry.headwords += [var.pali_1]
        dict_entry.headwords_count = len(dict_entry.headwords)
        dict_entry.headwords_clean.add(var.pali_clean)
...
def dict_entry_printer(counter, i: DictEntry) -> None:
    ...
    for var in i.variants:
        print(f"{var.pali_1:<40}[cyan]{var.pos:<10}[green]{var.meaning[:49]:<50}[white]{var.construction}")
    print()


def printer(item) -> None:
    print(f"{item.pali_1:<40}[cyan]{item.pos:<10}[green]{item.meaning[:49]:<50}[white]{item.construction}")
...
def main():
    ...
    # if constrction_clean in dict, and if pali_clean not in dict, update dict
