...
def family_root_contains_plus():
	...
	filtered_db = db_session.query(PaliWord).filter(PaliWord.family_root.contains("+"))

	for i in filtered_db:
		i.family_root = re.sub(" \\+ ", " ", i.family_root)
		db_session.commit()
...
def family_compound_contains_plus():
	...
	filtered_db = db_session.query(PaliWord).filter(
		PaliWord.family_compound.contains("+"))

	for i in filtered_db:
		i.family_compound = re.sub(" \\+ ", " ", i.family_compound)
		db_session.commit()
...
def wrong_pos():
	...
	filtered_db = db_session.query(PaliWord).filter(PaliWord.pos.notin_(pos_list))
	if filtered_db.count() > 0:
		for i in filtered_db:
