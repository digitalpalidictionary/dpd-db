...
def write_tsv(name_dict):
    with open("temp/theris.tsv", "w") as f:
        ...
        for number, i in name_dict.items():
            ...
            names = i["name"]
            for name in names:
                pali_1, pali_2, stem, pattern = name_stem_pattern(name)
                add = ""
                pos = "fem"
                grammar = "fem"
...
def name_stem_pattern(name):
    if name.endswith("dhītā"):
        pali_1 = name.replace("dhītā", "dhītar")
        pali_2 = name
        stem = name[:-1]
        pattern = "ar fem"
    elif name.endswith("dhītu"):
        pali_1 = name.replace("dhītu", "dhītar")
        pali_2 = name.replace("dhītu", "dhītā")
        stem = name[:-1]
        pattern = "ar fem"
    elif name.endswith("mātā"):
        pali_1 = name.replace("mātā", "mātar")
        pali_2 = name
        stem = name[:-4]
        pattern = "mātar fem"
    elif name.endswith("mātu"):
        pali_1 = name.replace("mātu", "mātar")
        pali_2 = name.replace("mātu", "mātā")
        stem = name[:-4]
        pattern = "mātar fem"
    else:
        pali_1 = name
    ...
    return pali_1, pali_2, stem, pattern
