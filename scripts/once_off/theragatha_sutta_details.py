""""Find names, chapterse and verses of Theragātha titles,
...
def sutta_chapter_name_dict(name_list):
    ...
    for i in name_list:
        ...
        if name == "vaṅgīsattheragāthā":
            dict[name]["last_verse"] = 1288
        previous_name = name
...
def add_to_db(name_dict):
    ...
    count = 1
    for i in db:
        if i.pali_1 in name_dict:
            print(i.pali_1)
            print(i.meaning_2)
            x = dotdict(name_dict[i.pali_1])
            string = f"Theragāthā {x.sutta}, "
