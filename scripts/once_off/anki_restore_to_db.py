...
def check_unique(db_session, i):
    # Check if id and pali_1 already exist in the database
    existing_word = db_session.query(PaliWord)\
        .filter((PaliWord.id == i.id) | (PaliWord.pali_1 == i.pali_1)).first()
...
def update_fields(id: str, i: PaliWord, data_dict: dict):
    changed_flag = False
    for attr in dir(i): 
