...
def zip_up_directory(input_dir: Path, compression_level: int = 9, delete_original: bool = False):
    """
    ...
    with zipfile.ZipFile(str(output_file), 'w', compression=zipfile.ZIP_DEFLATED, compresslevel=compression_level) as zip_file:
        for root, _, files in os.walk(input_dir):
            for file in files:
                ...
                zip_file.write(str(file_path), str(file_path.relative_to(input_dir.parent)))
