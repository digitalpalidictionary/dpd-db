...
var template = { 
	"family_compounds":"			<!-- in the if condition the variable 'data.compounds.length' is a workaround. normaly it would be only 'compounds.length' but the if part \
	...
	"feedback":"			<p>ID <b>{{id}}</b>\
				...
				<p><a class=link \
						...
						available.</span> \
				<p><img src='dpd.png'> ",
	"family_root":"			<p class='heading underlined'><b>{{count}}</b> words belong to the root family <b>{{root_family}}</b> ({{root_meaning}}) \
				<table class=family> \
					{{loop='data'}} \
					<tr> \
						<th>{{0}} \
	...
	",
	"family_idioms": " {{if='data.idioms.length >= 1'}}  \
				<p class=heading id='{{link}}_cf_top'>jump to: \
					{{loop='idioms'}} <a class=jump href=#{{link}}_cf_{{item}}>{{item}}</a> {{/loop}}\
				{{/if}}\
				\
				{{idiom_sub}}\
	\
				</table> \
				<p class=footer>Please add more idioms <a class=link \
						href=https://docs.google.com/forms/d/e/1FAIpQLSf9boBe7k5tCwq7LdWgBHHGIPVc4ROO5yjVDo1X5LDAxkmGWQ/viewform?usp=pp_url&entry.438735500={{link}}&entry.326955045=Idioms&entry.1433863141=GoldenDict+{{date}} \
						target=_blank>here</a>.\
						<a class='jump' href='#{{link}}_cf_top'> ⤴</a>",
	"idiom_sub":"			<p class='heading underlined' id='{{link}}_cf_{{idiom_name}}'><b>{{count}}</b> idiomatic expression which contains <b>{{idiom_name}}</b> <a class=jump href=#{{link}}_cf_{{idiom_name}}> ⤴</a>\
				\
				<table class=family> \
					{{loop='data'}} \
					<tr> \
