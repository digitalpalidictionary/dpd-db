            ...
            if i.meaning_1 and i.pos not in pos_exclude_list:
                ...
                for meaning in meanings_list:
                    if meaning in epd.keys() and not i.plus_case:
                        epd_string = f"{epd[meaning]}<br><b class='dpd'>{i.lemma_clean}</b> {i.pos}. {i.meaning_1}"
                        epd[meaning] = epd_string

                    if meaning in epd.keys() and i.plus_case:
                        epd_string = f"{epd[meaning]}<br><b class='dpd'>{i.lemma_clean}</b> {i.pos}. {i.meaning_1} ({i.plus_case})"
                        epd[meaning] = epd_string

                    if meaning not in epd.keys() and not i.plus_case:
                        epd_string = (
                            f"<b class='dpd'>{i.lemma_clean}</b> {i.pos}. {i.meaning_1}"
                        )
                        epd.update({meaning: epd_string})

                    if meaning not in epd.keys() and i.plus_case:
                        epd_string = f"<b class='dpd'>{i.lemma_clean}</b> {i.pos}. {i.meaning_1} ({i.plus_case})"
                        epd.update({meaning: epd_string})
        ...
        if lang == "en":
            ...
            for root_meaning in root_meanings_list:
                if root_meaning in epd.keys():
                    epd_string = f"{epd[root_meaning]}<br><b class='dpd'>{i.root}</b> root. {i.root_meaning}"
                    epd[root_meaning] = epd_string

                if root_meaning not in epd.keys():
                    epd_string = f"<b class='dpd'>{i.root}</b> root. {i.root_meaning}"
    ...
    for combined_number in combined_numbers:
        if combined_number:
            ...
            if make_link and number_link:
                ...
                if lang == "en":
                    epd_string = f"<b class='dpd'>{i.lemma_clean}</b>. {i.meaning_2} {anchor_link}"
                elif lang == "ru":
                    epd_string = f"<b class='dpd'>{i.lemma_clean}</b>. {i.meaning_2} {anchor_link}"
            else:
                if lang == "en":
                    epd_string = f"<b class='dpd'>{i.lemma_clean}</b>. {i.meaning_2}"
                elif lang == "ru":
                    epd_string = f"<b class='dpd'>{i.lemma_clean}</b>. {i.meaning_2}"
