...
class DataAndLogic:
    ...
    def should_process(self, i: DpdHeadword):
        return bool(
            i.meaning_1
            and i.lemma_clean not in self.exceptions
            and i.pos not in ["sandhi", "idiom"]
            and not i.family_compound
            and re.findall(r"^(su|dur|sa|nir)\b ", i.construction)
...
def add_fc_su_dur(e: ft.Event[ft.ListTile], page: ft.Page, right_panel: ft.Container):
