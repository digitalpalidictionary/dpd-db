...
def timed_db(self):
    s = time.time(); orig_db(self); stamp(f"database loaded      (took {time.time()-s:5.2f}s)")
gm.App._initialize_db_in_background = timed_db

orig_ensure = gm.App._ensure_tab_built
def timed_ensure(self, index):
    if index in self._mounted_tabs: return
    s = time.time()
...
def timed_warm(self):
    if NO_WARMUP:
        ...
        return
    s = time.time(); orig_warm(self); stamp(f"ALL TABS WARMED      (took {time.time()-s:5.2f}s)")
