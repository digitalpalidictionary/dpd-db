...
from export_roots_ru import generate_root_ru_html
from export_rpd import generate_epd_html
...
from mdict_exporter_ru import export_ru_to_mdict
...
def main():
    ...
    root_data_list, sizes = generate_root_ru_html(db_session, pth, dpspth, roots_count_dict)
    rendered_sizes.append(sizes)
    ...
    epd_data_list, sizes = generate_epd_html(db_session, pth, dpspth)
    rendered_sizes.append(sizes)
    ...
    export_to_goldendict(pth, dpspth, combined_data_list)
    ...
    if make_mdct:
        ...
        export_ru_to_mdict(combined_data_list, dpspth)

    toc()
    time_log.log("exporter.py::main() return")


def export_to_goldendict(pth: ProjectPaths, dpspth: DPSPaths, data_list: list) -> None:
    ...
    export_words_as_stardict_zip(data_list, ifo, dpspth.dpd_ru_zip_path, pth.icon_path)
