...
def create_inflection_table_index():
    ...
    inflection_table_index_dict = dict(
        zip(inflection_table_index_df.iloc[:, 0],
            inflection_table_index_df.iloc[:, 2]))

    return inflection_table_index_df, inflection_table_index_length,
    inflection_table_index_dict


inflection_table_index_df, inflection_table_index_length, \
    inflection_table_index_dict = create_inflection_table_index()

index = inflection_table_index_df.drop(
    ["cell range", "irreg", "aka", "aka cell range"], axis=1)
...
def generate_inflection_tables_dict(
    ...
    for row in range(inflection_table_index_length):
        ...
        inflection_table_df_filtered = inflection_table_df.loc[
        ...
        inflection_tables_dict[
            inflection_name]["df"] = inflection_table_df_filtered
