...
def main() -> None:
    ...
    if config_test("anki", "update", "yes"):
        ...
        # root matrix
        anki_data_list = make_anki_matrix_data(html_dict, db_session)
        deck = ["Root Matrix"]
        family_updater(anki_data_list, deck)
...
def make_root_header(rf_dict: dict[str, dict], rf: str) -> str:
    ...
    header += f"<b>{rf_dict[rf]['root_family']}</b> ({rf_dict[rf]['root_meaning']})</p>"
    return header
...
def add_rf_to_db(db_session: Session, rf_dict: dict[str, dict]) -> None:
    ...
    db_session.query(FamilyRoot).delete()
...
def update_lookup_table(db_session: Session) -> None:
    """Add root keys data to lookup table."""

    pr.green_tmr("adding roots to lookup table")

    r2h_dict: defaultdict[str, set[str]] = defaultdict(set)
    roots_db = db_session.query(DpdRoot).all()
    for r in roots_db:
...
def make_anki_matrix_data(
    html_dict: dict[str, str], db_session: Session
) -> list[tuple[str, str]]:
